...
export class TripAddComponent {
  ...
  constructor (fB: FormBuilder, private inputService: DbaseService) {
    this.inputService.getTrips().subscribe((sub: any) => {
      this.tripsList= sub
      this.newId= 0;
      for (let trip of this.tripsList)
        this.newId= Math.max(this.newId, trip.id + 1);
    });
  ...
  addTrip(): void {
    ...
    let newTrip= {
      ...
      photoLinks: ["../../assets/images/no_photo.png"]
    } as Trip;
    ...
    this.inputService.addNewTrip(newTrip);
